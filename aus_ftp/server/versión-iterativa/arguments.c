...
// --- Definición de opciones de línea de comandos ---
...
// --- Verifica si una IP pertenece a una interfaz de red local ---
static int is_valid_local_ip(const char *ip_str)
{
  ...
  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
  {
    if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET)
    {
      ...
      // Compara la IP del usuario con las locales
...
// --- Función que interpreta los argumentos pasados al programa ---
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
  {
  // Opción explícita --port o -p
  case 'p':
    args->port = atoi(arg);
    args->port_set = 1;
    break;

  // Opción explícita --address o -a
  case 'a':
  ...
  // Argumentos posicionales sin bandera (por orden)
  case ARGP_KEY_ARG:
  ...
  // Validaciones al finalizar el parseo
  case ARGP_KEY_END:
    // El puerto 21 solo puede ser usado por root
    if (args->port == 21 && geteuid() != 0)
    {
      argp_error(state, "Permission denied: Port 21 requires root privileges");
    }

    // Rango válido de puertos no privilegiados
    ...
    // Verifica que la dirección IP sea de una interfaz local
...
// --- Definición del parser de argumentos usando argp ---
static struct argp argp = {options, parse_opt, args_doc, doc, NULL, NULL, NULL};

// --- Función pública para inicializar y parsear argumentos ---
